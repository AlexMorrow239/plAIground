...
def get_session_files() -> List[Path]:
    ...
    return session_files + env_files
...
def cleanup_session_files(session_id: str) -> None:
    ...
    files_to_remove = [
        deployment_dir / f"session_{session_id}.json",
        deployment_dir / f".env.{session_id}"
    ]
...
def cleanup_expired_sessions(dry_run: bool = False) -> None:
    ...
    session_files = [f for f in get_session_files() if f.name.startswith('session_')]
    expired_count = 0
