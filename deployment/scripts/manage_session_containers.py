...
def load_session_config(session_id: str) -> Optional[Dict[str, Any]]:
    ...
    session_file = deployment_dir / f"session_{session_id}.json"
...
def start_session(session_id: str) -> bool:
    ...
    env_file = deployment_dir / f".env.{session_id}"
...
def main():
    ...
    session_file = deployment_dir / f"session_{args.session_id}.json"
    if not session_file.exists() and not args.logs:  # logs can work without config file
